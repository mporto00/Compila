...
public class PrintTree {
	...
	public void printClassBodyNode(ClassBodyNode x) {
		...
		System.out.println();
		System.out.print(
				x.number + ": ClassBodyNode ===> " + ((x.clist == null) ? "null" : String.valueOf(x.clist.number)) + " "
	...
	public void printMethodDeclNode(MethodDeclNode x) {
		...
		System.out.println();
		System.out.print(x.number + ": MethodDeclNode ===> " + x.position.image + " "
				+ ((x.dim == 0) ? "" : ("[" + x.dim + "] ")) + x.name.image + " " + x.body.number);
		printMethodBodyNode(x.body);
	}
	...
	public void printMethodBodyNode(MethodBodyNode x) {
		...
		System.out.print(x.number + ": MethodBodyNode ===> "
				+ ((x.param == null) ? "null" : String.valueOf(x.param.number)) + " " + x.stat.number);
	...
	// --------------------------- Chamada de metodo ----------------------
	private void numberMethodCallNode(MethodCallNode x) {
		if (x == null) {
			return;
		}

		x.number = kk++;
		numberStatementListNode(x.args);
	}

	private void printMethodCallNode(MethodCallNode x) {
		if (x == null) {
			return;
		}

		System.out.println();
		System.out.print(x.number + ": MethodCallNode ===> " + x.args);

		printVarListNode(x.args);
	}
	...
	public void printRelationalNode(RelationalNode x) {
		...
		if (x.expr1 != null && x.expr2 != null) {
			...
			if(x.expr1 == null && x.expr2 == null) {
				System.out.print(x.number + ": RelationalNode ===> null null");
			} else {
				if(x.expr1 != null) {
					System.out.print(x.number + ": RelationalNode ===> " + x.expr1.number +  " null" );
					printExpreNode(x.expr1);
				} else {
					System.out.print(x.number + ": RelationalNode ===> " +  " null"  + x.expr2.number );
					printExpreNode(x.expr2);
				}
			}
		}
	...
	public void printMultNode(MultNode x) {
		...
		System.out.println();
		System.out
				.print(x.number + ": MultNode ===> " + x.expr1.number + " " + x.position.image + " " + x.expr2.number);
		printExpreNode(x.expr1);
		printExpreNode(x.expr2);
	}
	...
	public void printVarNode(VarNode x) {
		...
		System.out.println();
		System.out.print(
				x.number + ": VarNode ===> " + x.position.image + " " + ((x.dim == 0) ? "" : ("[" + x.dim + "]")));
	}
